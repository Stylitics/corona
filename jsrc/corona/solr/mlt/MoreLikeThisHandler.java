/* [AM] NOTE: this handler has solr-core dependency with which lein
              tries to find tools.jar from JDK.
              So if lein uses not JDK it will result to error.
              To fix it one can set :java-cmd in global lein profile
              or $LEIN_JAVA_CMD envvar.
*/
/*
...
public class MoreLikeThisHandler extends RequestHandlerBase {
    ...
    public void handleRequestBody(SolrQueryRequest req, SolrQueryResponse rsp) throws Exception {
        ...
        try {
            ...
            RankQuery rankQuery = null;

            String rq = params.get(CommonParams.RQ);

            if (rq != null) {
                //log.info("rq:" + rq);
                //log.info("params:" + params.get(QueryParsing.DEFTYPE));
                QParser rqParser = QParser.getParser(rq, defType, req);
                rankQuery = (RankQuery) rqParser.getQuery();
                //log.info("rankQuery:" + rankQuery.toString());
            }

            String[] bfs = params.getParams("bf");
            log.info("bf:" + bfs);
            ...
            MoreLikeThisHelper mlt = new MoreLikeThisHelper(params, searcher, req);
            ...
            try {
                ...
                // --------------------------------------------------------------------------------
                if (reader != null) {
                    mltDocs = mlt.getMoreLikeThis(reader, start, rows, rankQuery, sortSpec, filters, interesting, flags);
                } else if (q != null) {
                    // Matching options
                    ...
                    if (iterator.hasNext()) {
                        ...
                        mltDocs = mlt.getMoreLikeThis(id, start, rows, rankQuery, sortSpec, filters, interesting, flags);
                    }
                } else {
                    throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
    ...
    public static class MoreLikeThisHelper {
        ...
        public MoreLikeThisHelper(SolrParams params, SolrIndexSearcher searcher, SolrQueryRequest request) {
        ...
        protected void addBoostFunctions(BooleanQuery.Builder query, int flags) throws SyntaxError {
            String[] boostFuncs = this.params.getParams(DisMaxParams.BF);
            if (null != boostFuncs && 0 != boostFuncs.length) {
                for (String boostFunc : boostFuncs) {
                    if (null == boostFunc || "".equals(boostFunc)) continue;
                    Map<String, Float> ff = SolrPluginUtils.parseFieldBoosts(boostFunc);
                    for (String f : ff.keySet()) {
                        QParser nestedParser = QParser.getParser(f, FunctionQParserPlugin.NAME, this.request);
                        nestedParser.setFlags(flags);
                        Query fq = nestedParser.getQuery();
                        Float b = ff.get(f);
                        if (null != b) {
        ...
        public DocListAndSet getMoreLikeThis(int id, int start, int rows, RankQuery rankQuery, SortSpec sortSpec, List<Query> filters, List<InterestingTerm> terms, int flags) throws IOException, SyntaxError {
            ...
            addBoostFunctions(realMLTQuery, flags);
            ...
            log.info("---------------------ID---------------------MoreLikeThisHelper.getMoreLikeThis");
            log.info("boostedMLTQuery: "+boostedMLTQuery.toString());
            this.queryToUse = this.realMLTQuery;
            if (rankQuery != null) {
                this.queryToUse = rankQuery.wrap(this.realMLTQuery);
            }

            if (this.needDocSet) {
                results = searcher.getDocListAndSet(this.queryToUse, filters, sortSpec.getSort(), start, rows, flags);
            } else {
                results.docList = searcher.getDocList(this.queryToUse, filters, sortSpec.getSort(), start, rows, flags);
            }

            return results;
        }

        public DocListAndSet getMoreLikeThis(Reader reader, int start, int rows, RankQuery rankQuery, SortSpec sortSpec, List<Query> filters, List<InterestingTerm> terms, int flags) throws IOException {
            ...
            //log.info("---------------------READER---------------------MoreLikeThisHelper.getMoreLikeThis");
            this.queryToUse = boostedMLTQuery;
            if (rankQuery != null) {
                this.queryToUse = rankQuery.wrap(boostedMLTQuery);
            }
